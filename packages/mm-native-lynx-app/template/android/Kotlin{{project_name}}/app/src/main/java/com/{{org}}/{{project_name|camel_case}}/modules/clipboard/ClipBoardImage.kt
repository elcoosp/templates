...
data class ImageResult(val base64Image: String, val width: Int, val height: Int) {
    fun toBundle() =
            bundleOf(
                    "data" to base64Image,
                    "size" to bundleOf("width" to width, "height" to height)
            )
}
// endregion
...
/**
 * Gets the image format from the base64 encoded image string by checking the prefix of the base64
 *
...
/**
 * Gets the [imageUri] and returns the [ImageResult] object containing base64 encoded image and its
 * metadata
 *
 * @param context
...
internal suspend fun imageFromContentUri(
    ...
    val outputStream =
            ByteArrayOutputStream().also { bitmap.compress(format.compressFormat, quality, it) }
    yield()
...
/**
 * Gets base64-encoded image data and prepares it to be accessible by other applications when
 * pasting from clipboard
 *
 * Saves the image to the [clipboardCacheDir] directory, then uses [ClipboardFileProvider] to create
 * a `content://` URI which then is packed to the [ClipData] object.
 *
 * @param context
 * @param base64Image base64-encoded JPEG image data. Should not be prefixed
 * @param clipboardCacheDir directory where the copied image is stored, must be accessible by the
...
internal suspend fun clipDataFromBase64Image(
    ...
    val file = File(clipboardCacheDir, fileName).also { it.ensureExists() }
...
internal fun bitmapFromBase64String(base64Image: String): Bitmap =
        try {
